...
export const createLecturerQuizSchema = z.object({
  level: z.number({ message: "Level wajib diisi" }).int().min(1, "Level minimal 1"),
  title: z.string({ message: "Judul wajib diisi" }).min(1, "Judul kuis wajib diisi"),
...
export const lecturerQuestionSchema = z.object({
  question_text: z.string({ message: "Pertanyaan wajib diisi" }).min(1, "Pertanyaan wajib diisi"),
  key_answer_text: z.string({ message: "Kunci jawaban wajib diisi" }).min(1, "Kunci jawaban wajib diisi"),
