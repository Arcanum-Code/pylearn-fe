...
import {
  useLecturerQuizDetail,
  usePublishLecturerQuiz,
  useDeleteLecturerQuiz,
...
export function LecturerQuizDetail({
  groupId,
  quizId,
}: LecturerQuizDetailProps) {
  const router = useRouter();
  const { data: quiz, isLoading } = useLecturerQuizDetail(quizId);
  const { mutate: publishQuiz, isPending: isPublishing } =
    usePublishLecturerQuiz(groupId, quizId);
  const { mutate: deleteQuiz, isPending: isDeleting } =
    useDeleteLecturerQuiz(groupId);
  ...
  const handleDelete = async () => {
    const isConfirmed = await confirm({
      title: "Hapus Kuis?",
      description:
        "Apakah Anda yakin ingin menghapus kuis ini? Semua data pertanyaan di dalamnya akan hilang.",
      confirmText: "Hapus",
      cancelText: "Batal",
      variant: "destructive",
    });
  ...
  const handlePublish = async () => {
    const isConfirmed = await confirm({
      ...
      description:
        "Kuis yang sudah dipublikasikan tidak dapat diedit lagi. Apakah Anda yakin ingin mempublikasikan kuis ini?",
      ...
      <div className="sm:p-2 md:p-4 max-w-7xl mx-auto space-y-6 bg-[#F7F8FA] min-h-screen font-sans text-[#1A1C1E]">
            ...
            {quiz.status === "draft" && (
              <Tooltip>
                ...
                {quiz.can_publish === false && (
                  <TooltipContent>
                    Grup ini belum memiliki materi yang dipublikasikan, sehingga
                    kuis ini tidak dapat dikunci.
                  </TooltipContent>
                )}
              </Tooltip>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-xs border border-gray-150/60">
              <div className="flex items-center justify-between mb-6">
                ...
                <Tooltip>
                  ...
                  {quiz.status === "published" && (
                    <TooltipContent>
                      Kuis yang sudah dipublikasikan tidak dapat ditambahkan
                      soal baru.
                    </TooltipContent>
                  )}
                </Tooltip>
              ...
              <div className="space-y-4">
                {!quiz.questions || quiz.questions.length === 0 ? (
                  ...
                  quiz.questions.map((q) => (
                    <div
                      key={q.question_id}
                      className="p-5 rounded-xl bg-[#F7F8FA] border border-gray-150/40 space-y-3"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="space-y-1">
                          <span className="font-mono text-xs text-[#6366F1] font-semibold">
                            Soal {q.sequence_order}
                          </span>
                        ...
                        <div className="flex gap-1">
                          <Tooltip>
                            ...
                            {quiz.status === "published" && (
                              <TooltipContent>
                                Kuis yang sudah dipublikasikan tidak dapat
                                diedit.
                              </TooltipContent>
                            )}
                          </Tooltip>
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-xs border border-gray-150/60">
              ...
              <div className="space-y-3">
                {!quiz.gating_materials ||
                quiz.gating_materials.length === 0 ? (
                  <p className="text-xs text-gray-400 font-mono">
                    Tidak ada materi prasyarat untuk kuis ini.
                  </p>
                ) : (
                  quiz.gating_materials.map((m) => (
                    <div
                      key={m.material_id}
                      className="flex justify-between items-center p-3 bg-gray-50 border rounded-lg text-xs"
                    >
        ...
        {questionModal.isOpen && (
          <LecturerQuestionForm
            ...
            onOpenChange={(open) =>
              !open && setQuestionModal({ isOpen: false, mode: "create" })
            }
            groupId={groupId}
            quizId={quizId}
