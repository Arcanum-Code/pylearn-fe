...
export function LecturerDashboardContainer() {
  ...
  return (
    <div className="space-y-6">
      {/* Action Alerts Banner at the true top */}
      <ActionAlertsBanner />
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-150/60 shadow-xs">
            <div>
              <h2 className="text-xl font-bold tracking-tight">
                {selectedGroupId === "all"
                  ? "Ringkasan Umum"
                  : `Ringkasan: ${groups?.find((g) => g.id === selectedGroupId)?.name || "Detail Kelas"}`}
              </h2>
                ...
                <TooltipProvider delayDuration={100}>
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom" className="max-w-[250px] text-center">
                      <p>Pilih kelas spesifik pada sidebar untuk mengelola data kelas tersebut.</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              ) : (
